...
namespace DStream.Net
{
    ...
    public class SQLServerMonitor
    {
        ...
        public SQLServerMonitor(SqlConnection dbConn, string tableName, TimeSpan initialInterval, TimeSpan maxInterval, MonitoringCallback callback, ILogger<SQLServerMonitor> logger, ILoggerFactory loggerFactory)
        {
            _dbConn = dbConn;
            _backoffManager = new BackoffManager(initialInterval, maxInterval);

            // Create a logger for LSNManager using loggerFactory
            _lsnManager = new LSNManager(dbConn, tableName, loggerFactory.CreateLogger<LSNManager>());

            _checkpointManager = new CheckpointManager(dbConn, tableName);
            _callback = callback;
            _logger = logger;
        ...
        public async Task MonitorTableAsync(TableConfig tableConfig, CancellationToken cancellationToken)
        {
            ...
            while (!cancellationToken.IsCancellationRequested)
            {
                try
                {
                    var columns = await DatabaseMetadataHelper.GetColumnNamesAsync(_dbConn, "dbo", tableConfig.Name);
